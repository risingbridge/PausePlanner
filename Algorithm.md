...
PausePlanner supports four scheduling algorithms — see `src/scheduler/index.ts` for the pluggable registry (`AlgorithmId`/`ALGORITHMS`) and the "Scheduling algorithm" setting on the Settings page for how one is chosen. This document covers **Quick**, the fast greedy first mode this project shipped (no longer the default — see [Algorithm-Mip.md](Algorithm-Mip.md) for **MIP (HiGHS)**, which is). See [Algorithm-Balanced.md](Algorithm-Balanced.md) for **Balanced**, which trades speed for better break placement in tightly-staffed days; [Algorithm-Thorough.md](Algorithm-Thorough.md) for **Thorough**, which solves coverage to proven optimality via a joint branch-and-bound search; and [Algorithm-Refine.md](Algorithm-Refine.md) for **Refine**, which polishes a Quick-seeded schedule via simulated annealing.
