...
export interface RefineWorkerRequest {
  positions: Position[];
...
export type RefineWorkerResponse = { ok: true; result: ScheduleResult } | { ok: false; error: string };
...
interface WorkerGlobal {
  onmessage: ((ev: MessageEvent<RefineWorkerRequest>) => void) | null;
  postMessage: (data: RefineWorkerResponse) => void;
}
...
ctx.onmessage = (event) => {
  try {
    ...
    const result = runRefine(positions, openings, staff, settings);
