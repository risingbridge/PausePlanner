...
export interface RotateExperimentalWorkerRequest {
  positions: Position[];
...
export type RotateExperimentalWorkerResponse = { ok: true; result: ScheduleResult } | { ok: false; error: string };
...
interface WorkerGlobal {
  onmessage: ((ev: MessageEvent<RotateExperimentalWorkerRequest>) => void) | null;
  postMessage: (data: RotateExperimentalWorkerResponse) => void;
}
...
ctx.onmessage = (event) => {
  try {
    ...
    const result = runRotateExperimental(positions, openings, staff, settings);
