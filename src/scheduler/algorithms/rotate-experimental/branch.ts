...
function legalOptionsFor(
  ...
  if (freeToChoose) {
    ...
    // legal rather than deferring it — inherited from Thorough
    // (Experimental); do not reorder this as a side effect of the rotation
    // objective (see Algorithm-RotateExperimental.md). Position balance
    // influences which branch *scores* best, not the order branches are
    // tried.
...
// ever discarding a genuinely distinct outcome. Soundness now depends on
// stateSignature including positionMinutes (see state.ts) — two people
// with different position histories are never "identical" here.
...
// Returns a copy of positionMinutes with `positionId`'s slot credited —
// the one place a person's cumulative position matrix actually grows.
function creditPosition(positionMinutes: number[], positionId: string, ctx: SearchContext, minutes: number): number[] {
  const idx = ctx.positionIndexById.get(positionId)!;
  const next = positionMinutes.slice();
  next[idx] += minutes;
  return next;
}
...
export function enumerateSlotOutcomes(
  ...
  // rather than silently letting the bounce through. Required time credits
  // positionMinutes exactly like freely-chosen work — this is what makes
  // the rotation objective aware a requirement window happened at all
  // (Algorithm-RotateExperimental.md's "required minutes must count toward
  // the matrix").
  ...
  for (const [i, positionId] of requiredByIndex) {
    ...
    baseNextStates[i] = {
      ...
      elapsedMinutes: st.elapsedMinutes + slotMinutes,
      positionMinutes: creditPosition(st.positionMinutes, positionId, ctx, slotMinutes),
    };
  }
    ...
    for (const i of freeIndices) {
      ...
      if (opt.kind === "work") {
        ...
        nextStates[i] = {
          ...
          elapsedMinutes: st.elapsedMinutes + slotMinutes,
          positionMinutes: creditPosition(st.positionMinutes, opt.positionId, ctx, slotMinutes),
        };
