...
export interface ThoroughWorkerRequest {
  positions: Position[];
...
export type ThoroughWorkerResponse = { ok: true; result: ScheduleResult } | { ok: false; error: string };
...
interface WorkerGlobal {
  onmessage: ((ev: MessageEvent<ThoroughWorkerRequest>) => void) | null;
  postMessage: (data: ThoroughWorkerResponse) => void;
}
...
ctx.onmessage = (event) => {
  try {
    ...
    const result = runThorough(positions, openings, staff, settings);
