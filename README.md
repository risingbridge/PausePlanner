...
- **Thorough (Experimental)** — a fork of Thorough kept as a standing incubator for experimental features, not a fixed fifth mode. Currently the only algorithm that honors **required positions** (Staffing page): a staff member forced into a specific position for a specific window. The other algorithms leave required positions as inert data — the Schedule page warns when they exist but the selected algorithm won't enforce them.
