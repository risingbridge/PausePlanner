...
The scheduling algorithm is chosen on the Settings page. There are seven:

- **Quick** — fast, greedy, walks the day forward one slot at a time and never looks back.
...
- **Thorough (Experimental)** — a fork of Thorough kept as a standing incubator for experimental features, not a fixed fifth mode. Honors **required positions** (Staffing page): a staff member forced into a specific position for a specific window. Algorithms that don't honor requirements leave them as inert data — the Schedule page warns when they exist but the selected algorithm won't enforce them.
- **Rotate (Experimental)** — a fork of Thorough (Experimental), keeping everything it does (including required positions) and adding a fair-rotation objective that spreads each position's time evenly across the staff, so nobody gets typecast onto one station. Trades away some of Thorough's minimal-churn preference for that variety — see the doc for why.
- **MIP (HiGHS)** — a completely different engine from the other six: formulates coverage, every labor rule, and a staged fairness objective as a mixed-integer linear program and solves it with [HiGHS](https://highs.dev/), a real branch-and-cut solver compiled to WebAssembly. Its ~3.4MB solver is loaded lazily, only when this mode is actually selected and run, so it costs nothing for anyone using the other six. Also honors required positions, and comes with a genuine optimality proof rather than a search budget running out — it's the default.

See [Algorithm.md](Algorithm.md) for a full, detailed walkthrough of Quick, [Algorithm-Balanced.md](Algorithm-Balanced.md) for Balanced, [Algorithm-Thorough.md](Algorithm-Thorough.md) for Thorough, [Algorithm-Refine.md](Algorithm-Refine.md) for Refine, [Algorithm-ThoroughExperimental.md](Algorithm-ThoroughExperimental.md) for Thorough (Experimental), [Algorithm-RotateExperimental.md](Algorithm-RotateExperimental.md) for Rotate (Experimental), and [Algorithm-Mip.md](Algorithm-Mip.md) for MIP (HiGHS). Quick summary below.
