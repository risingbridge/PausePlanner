...
- **Settings** — tune the scheduling rules: minimum position length, max time in position, minimum break length, minimum idle time, and the earliest/latest points (as % of shift) the one real break can land. Also where you export/import everything as a JSON file, to back up your work or move it to another computer.
