...
The scheduling algorithm is chosen on the Settings page. There are five:
...
- **Thorough (Experimental)** — a fork of Thorough kept as a standing incubator for experimental features, not a fixed fifth mode. Currently the only algorithm that honors **required positions** (Staffing page): a staff member forced into a specific position for a specific window. The other four algorithms leave required positions as inert data — the Schedule page warns when they exist but the selected algorithm won't enforce them.

See [Algorithm.md](Algorithm.md) for a full, detailed walkthrough of Quick, [Algorithm-Balanced.md](Algorithm-Balanced.md) for Balanced, [Algorithm-Thorough.md](Algorithm-Thorough.md) for Thorough, [Algorithm-Refine.md](Algorithm-Refine.md) for Refine, and [Algorithm-ThoroughExperimental.md](Algorithm-ThoroughExperimental.md) for Thorough (Experimental). Quick summary below.
